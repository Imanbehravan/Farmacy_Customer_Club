...
from .models import Prescription, PrescriptionItem, INSURANCE_CHOICES, INSURANCE_BASE_KEYS, INSURANCE_SUPP_KEYS
...
class SubmitPrescriptionView(View):
    ...
    def post(self, request):
        ...
        insurance_type = request.POST.get('insurance_type', 'free')
        ...
        insurance_label = dict(INSURANCE_CHOICES).get(insurance_type, insurance_type)
        Notification.objects.create(
            user=request.user,
            title='نسخه دریافت شد',
            message=(
                f'نسخه با کد رهگیری {tracking_code} دریافت شد و در حال بررسی است.\n'
                f'نوع بیمه ثبت‌شده: {insurance_label}\n'
...
class AdminPrescriptionsView(View):
    ...
    def get(self, request):
        ...
        if insurance_filter:
            prescriptions = prescriptions.filter(insurance_type=insurance_filter)
...
class AdminPrescriptionDetailView(View):
    ...
    def post(self, request, pk):
        ...
        # ── Step 5c: Set contract status ──
        if action == 'set_contract':
            contract_value = request.POST.get('contract_value', 'unknown')
            if contract_value in ['contracted', 'no_contract', 'unknown']:
                prescription.contract_status = contract_value
                prescription.save(update_fields=['contract_status'])

                insurance_label = prescription.get_insurance_label()
                if contract_value == 'contracted':
                    notif_title = 'داروخانه با بیمه شما قرارداد دارد ✅'
                    notif_msg = (
                        f'بیمه {insurance_label} شما در سامانه داروخانه ثبت شده است.\n'
                        f'با ارائه دفترچه/کارت بیمه، از پوشش بیمه‌ای بهره‌مند خواهید شد.'
                    )
                else:
                    notif_title = 'اطلاعیه وضعیت بیمه ⚠️'
                    notif_msg = (
                        f'متأسفانه داروخانه با {insurance_label} قرارداد مستقیم ندارد.\n'
                        f'امکان ارائه خدمات بیمه‌ای برای این نسخه وجود ندارد. '
                        f'لطفاً برای اطلاعات بیشتر با داروخانه تماس بگیرید.'
                    )
                Notification.objects.create(
                    user=prescription.user,
                    title=notif_title,
                ...
                messages.success(request, f'وضعیت قرارداد بیمه ثبت شد و کاربر مطلع گردید.')
            return redirect('admin_prescription_detail', pk=pk)
